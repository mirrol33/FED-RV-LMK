...
// 로딩구역 만들기
// 대상 : window (최상위 JS 객체)
// 이벤트 등록 메서드 : addEventListener(이벤트,함수)
// -> 이벤트와 함수를 연결시키는 메서드
...
// 모두 로딩될 때까지 기다림!
// 2) DOMContentLoaded : HTML 태그만 다 불러오면 실행

// 로드
window.addEventListener('DOMContentLoaded', loadFn); //// 로드구역 ////
    
// 로딩실행 함수
// (지역변수로 써서 defer 전역변수 보다 더 안전함!)
function loadFn(){
    console.log("로딩완료!");
    // 0. 요구사항 분석:
    // 1) TV버튼 클릭시 기능을 수행함
    ...
    function controlTV(){
        ...
        // 2. 함수호출확인!
        console.log("TV틀어!", className, tv.style.width, scaleValue);
        // [CSS 스타일값 읽기방법 2가지]
        // 1) style.속성명 방법 (인라인 CSS만 읽어옴)
        // -> 왜 인라인만인가? style.속성명 = 값 설정을
        // 생각해보면 이것이 인라인 CSS를 설정하는 방법이기 때문
        ...
        switch(className){
            ...
            case "ch2": mv.src = "https://www.youtube.com/embed/3P1CnWI62Ik?autoplay=1"; break;
            case "ch3": mv.src = "https://www.youtube.com/embed/xLD8oWRmlAE?autoplay=1"; break;
        }
    } //// controlTV 함수 ////
