...
// span태그로 싸서 대상박스에 넣기
// 대상 : stage
stage.innerHTML = newText.map((v,i)=>`<span style="transition-delay:${i*0.1}s">${v}</span>`).join('');
...
// -> 원본 배열은 보존된다!
const aa = ['a','b','c','d','e','f'];
console.log('aa배열:',aa);
// 기존 배열값에 하트를 넣는다
console.log('하트넣은 aa배열:',aa.map(v=>'♥'+v));
console.log('하트넣은 aa배열을 문자열로 출력:',aa.map(v=>'♥'+v).join('zz'));
console.log('div태그로 aa배열을 싸고 문자열로 출력:',aa.map(v=>`<div>${v}</div>`).join(''));
