...
// 기준값 만들기 : 화면 높이값을 사용
const CRITERIA = (window.innerHeight / 3) * 2; // 상수값
...
function showEl() {
  ...
  scrollAct.forEach((el) => {
    ...
    // console.log(scrollAct[0].getBoundingClientRect().top);
    // console.log(myFn.getBCR(myscrollAct[0]));

    // 각 등장요소의 바운딩 top값
    let bcrVal = myFn.getBCR(el);
    // console.log(bcrVal, scrollAct[0].getBoundingClientRect());
...
function moveTit() {
  ...
  if (scY > posEl[2] - CRITERIA) tit.style.left = "30%"; // 3번째 요소
  else if (scY > posEl[1] - CRITERIA) tit.style.left = "78%";
  else if (scY > posEl[0] - CRITERIA) tit.style.left = "28%";
...
/****************************************************************** 
    ...
    (1) 기본원리 : 스크롤 이동에 따른 화면 높이값 범위안에서 
    떨어지는 여자 이미지가 아래쪽으로 이동 애니메이션 됨!

    (2) 계산을 위한 비례식 세우기
    스크롤 한계값 : 윈도우 높이값 = 스크롤 이동값 : 이미지 이동값
