...
//  -> 각 기능버튼에 클릭이벤트를 설정함
mbtn.forEach((el) => {
  myFn.addEvt(el, "click", showFruite);
}); /// forEach ///
function showFruite() {
  // (1) 버튼 텍스트 읽기
  ...
  if (btxt === "과일주세요~!") {
    ...
    ${
      ...
      fruit
        .map(
          (v) => `<li
        style="background: url(./addimg/${frObj[v]}.png) no-repeat center / cover">${v}</li>`
        )
        .join("")
    }
    </ul>
