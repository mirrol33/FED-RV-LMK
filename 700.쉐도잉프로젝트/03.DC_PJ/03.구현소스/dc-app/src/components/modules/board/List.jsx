...
function List({
  ...
  *******************************************/
  ...
  const reducerFn = (memory, action) => {
    ...
    // 2. 최신 검색어를 기준으로 5개 이상일 경우 맨 앞배열값 삭제하기
    let newArr = memory.split("*");
    if(newArr.length > 4) newArr.shift();
    // 맨앞 배열값 제거후 join으로 문자열 만들기
    newArr = newArr.join("*");
  ...
  // (3) 재검색할 경우 호출하여 리듀서변수값 유지 (구분값:'again')
  ...
  useEffect(()=>{
    // 컴포넌트 소멸시 구역
    return(()=>{
      //리듀서 검색어 저장값을 로컬스에 할당함
      localStorage.setItem("memory-data", memory);
      console.log('리듀서 검색어 저장값을 로컬스에 할당함!', memory);
    });
    // memory 변수 의존성을 심어서 만약 memory가 변경되면
    // 변경된 값을 반영한 소멸자가 구역 코드를 다시 구성함!
    // [] 빈 대괄호를 하여 처음한번실행코드를 만들면
