...
function Read({setMode, selRecord}) {
  ...
  if (myCon.loginSts) {
    // 로그인한 사용자정보가 할당된 경우 true 처리
    // console.log('선택글 아이디:', selData.uid, '로그인사용자 아이디:', myCon.loginSts.uid);
  ...
  if (!isRec) {
    ...
    // 조건 : isRec값이 false일때(여기 if문 내부에 코딩!)
    bdData.some((v) => {
