...
ReactDOM.render(<Developer isNormal={false} isrc={devImg[0]} ialt="개발자 공유" itit="프론트엔드 개발자 공유입니다!" />, root[0]);
...
function WishList({wList, gubun}) {
  ...
  return (
    <React.Fragment>
        ...
        wList.length > 0 && (
          <div>
            ...
            <ul>
              {
                // 구분값이 "음식"인 경우
                gubun == "음식" && wList.map((v) => <MakeList foodName={v} />)
              }
              {
                // 구분값이 "영화"인 경우
                gubun == "영화" && wList.map((v) => <MakeList movieInfo={v} />)
              }
            </ul>
            {
              // 구분값이 "영화"인 경우 포스터출력
              gubun == "영화" && wList.map((v) => <MakeImage isrc={v.poster} ialt={v.mtit} icss={{width: "16%", marginLeft: "0.5%"}} />)
            }
          </div>
        )
      ...
      {wList.length == 0 && <h2>아직 개발자 {gubun} 리스트가 업데이트 되지 않았습니다!</h2>}
    </React.Fragment>
  );
} /////////// WishList 컴포넌트 ////////
...
ReactDOM.render(<WishList wList={movs} gubun="영화" />, root[4]);

// 영화 위시리스트 출력하기 : 배열값이 없는 경우 ///
ReactDOM.render(<WishList wList={[]} gubun="영화" />, root[5]);
...
function MakePainting({painter, wname}) {
  ...
  return (
    <div>
      ...
      <h2> {painter} </h2>
      {/* 이미지 태그 출력 */}
      <img src={workUrl[painter]} alt={painter + "의 작품:" + wname} style={{width: "400px"}} title={wname} />
    </div>
  );
} //// MakePainting 컴포넌트 ////
...
function ShowLikePainter({isChange}) {
  ...
  // result는 읽기 전용변수
  // setResult는 쓰기 전용메서드-> setResult(값)
